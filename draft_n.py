    ...
    def execute(self, instr, current_pc):
        ...
        if opcode == 0b0110011:
            ...
            if opcode == 0b0010011:
                ...
                target = u32(a + imm) & ~1
                self.write_reg(rd, ret_addr)       # jalr
                self.pc = u32(target)
                return False
            ...
            self.memory.sw(address, self.read_reg(rs2))   # sw
            ...
            if funct3 == 0b000:
                ...
                take = (s32(a) < s32(b))           # blt
            elif funct3 == 0b101:
                take = (s32(a) >= s32(b))          # bge
            elif funct3 == 0b110:
            ...
            self.write_reg(rd, current_pc + 4)     # jal
            self.pc = u32(current_pc + imm)
            return False
...
try:
    ...
    try:
        while True:
            current_pc = cpu.pc
            instr = memory.read_instr(current_pc)
            halted = cpu.execute(instr, current_pc)
            cpu.add_trace()
            if halted:
                break

        output_lines = []
        output_lines.extend(cpu.trace_lines)

        for addr in range(DATA_MEM_BASE, DATA_MEM_BASE + DATA_MEM_WORDS * 4, 4):
            output_lines.append(f"0x{addr:08x}:{to_bin32(memory.data.get(addr, 0))}")

    except SimulationError:
        output_lines = cpu.trace_lines[:]
