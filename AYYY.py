...
DATA_MEM_WORDS = 32
DATA_MEM_END = DATA_MEM_BASE + 4 * DATA_MEM_WORDS - 4  # last valid word address
...
class CPU:
    def __init__(self, memory, addr_to_line):
        ...
        self.regs = [0] * 32
        self.regs[2] = 0x0000017C   # stack pointer initial value
        self.pc = 0
        self.trace_lines = []
    ...
    def run(self):
        while True:
            ...
            # trace only after successful execution of instruction
            self.trace()
...
def run_simulation_from_lines(lines):
    ...
    try:
        cpu.run()
        return cpu.trace_lines, memory.dump_data_memory_lines(), False
    except SimulationError:
        # stop immediately on runtime error
        # keep only traces till the last successfully executed instruction
        return cpu.trace_lines, [], True
...
def main():
    ...
    try:
        if len(sys.argv) == 1:
            ...
            trace_lines, memory_lines, _ = run_simulation_from_lines(lines)
        ...
        # if something goes wrong very early, still create empty output file in file mode
